...
// All images moved from the showcaseimages folder into the public directory
const images = [
...
export default function ShowcasePage() {
  // We duplicate the items array so the animation has a seamless infinite loop boundary
  ...
  return (
    <>
      ...
      <div className="absolute inset-0 flex gap-2 p-2 opacity-50 scale-110 md:-rotate-3 md:scale-125 pointer-events-none">
        ...
        {/* Column 3 - Fast Speed */}
        <div className="flex-1 flex flex-col gap-2 animate-scroll-up [animation-duration:15s] mt-5 min-w-[150px] md:min-w-[250px] hidden sm:flex">
          {items.map((src, i) => (
        ...
        <div className="flex-1 flex flex-col gap-2 animate-scroll-up [animation-duration:32s] -mt-20 min-w-[150px] md:min-w-[250px] hidden lg:flex">
          {items.slice().reverse().map((src, i) => (
        ...
        <div className="flex-1 flex flex-col gap-2 animate-scroll-up [animation-duration:22s] min-w-[150px] md:min-w-[250px] hidden xl:flex">
          {items.map((src, i) => (
