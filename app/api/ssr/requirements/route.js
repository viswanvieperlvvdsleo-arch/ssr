...
export async function POST(request) {
  ...
  let saved;
  try {
    // Token, internal post and task must either all exist or none exist.
    saved = await prisma.$transaction(async tx => {
      const post = await tx.appPost.create({ data: buildPostData({
        authorId: actor.id, authorName: actor.name, authorRole: actor.role,
        category: 'Announcements', title, content: body, visibility: 'internal',
        isRequirement: true, companyId: company?.id || null,
      }) });
      const task = await tx.appRequirementTask.create({ data: {
        postId: post.id, title, description: body, createdById: actor.id,
        createdByName: actor.name, companyId: company?.id || null,
      } });
      const token = `REQ-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${randomBytes(5).toString('hex').toUpperCase()}`;
      const submission = await tx.appRequirementSubmission.create({ data: {
        token, taskId: task.id, companyId: company?.id || null, senderId: actor.id,
        fromEmail: actor.email, cc, signature: signature || null, emailStatus: 'sending',
      } });
  ...
  const { task, submission } = saved;
  const notify = async () => {
    const staff = await prisma.appUser.findMany({
      where: { ...SJ_USER_FILTER, role: { in: ['Super Admin', 'Admin', 'Employee'] }, restricted: false }, select: { id: true },
    });
  ...
  const [mailResult] = await Promise.allSettled([
    sendRequirementEmail({ submission, task, company }), notify(),
    bumpRealtimeRevision('posts'), bumpRealtimeRevision('tasks'),
  ]);
  const mail = mailResult.status === 'fulfilled' ? mailResult.value : { emailStatus: 'unknown', emailError: 'Email confirmation unavailable.' };
  await prisma.appRequirementSubmission.update({ where: { id: submission.id }, data: mail }).catch(() => console.error('Could not persist requirement email status', submission.token));
  return NextResponse.json({ token: submission.token, taskId: task.id, postId: task.postId, ...mail }, { status: 201 });
}
