...
export function buildPostData(data = {}) {
  ...
  return {
    ...
    authorName: data.authorName || 'SJ INFO BUSINESS SOLUTIONS',
    authorTitle: data.authorTitle || data.authorRole || 'User',
    authorInitials: data.authorInitials || initialsFor(data.authorName || 'SJ INFO BUSINESS SOLUTIONS'),
...
export function buildMeetingData(data = {}) {
  return {
    ...
    module: data.module || 'General',
    hostId: data.hostId,
    meetingType: data.meetingType === 'external' ? 'external' : 'internal',
