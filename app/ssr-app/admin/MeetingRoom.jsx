...
export default function MeetingRoom({ meetingCode }) {
  ...
  const leaveMeeting = useCallback(async (navigate = true) => {
    ...
    setSession(null);
    if (navigate) router.replace('/ssr-app/admin/home?section=meetings');
  ...
  if (loadError || !meeting) return <div className={styles.statePage}><h1>Meeting unavailable</h1><p>{loadError || 'This meeting could not be found.'}</p><button onClick={() => router.replace('/ssr-app/admin/home?section=meetings')}>Back to meetings</button></div>;
  ...
  if (!session) {
    return (
      <div className={styles.prejoinPage}>
        <header><img src="/logo/192.png" alt="SJ INFO BUSINESS SOLUTIONS logo"/><strong>SJ Meet</strong><button onClick={() => router.replace('/ssr-app/admin/home?section=meetings')}>{Icons.close}</button></header>
