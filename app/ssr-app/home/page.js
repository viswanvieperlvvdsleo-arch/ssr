              ...
              return (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 'auto' }}>
                  <CallButton
                    ...
                    targetUserName={otherUser?.name || activeChat.name}
                    callType="audio"
                    currentUser={currentUser}
                    checkAccess={canDirectChatWith}
                    onRequestAccess={requestChatAccess}
                  />
                  <CallButton
                    targetUserId={otherUserId}
                    targetUserName={otherUser?.name || activeChat.name}
...
function AccountManagementPanel({ currentUser, onViewEmployeeChats }) {
  const { users, login, deleteUser, restrictUser, addEmployee, addAdmin, updateUserPermissions, updateEmployeeProfile, viewUserProfile, posts, courses } = useApp();
  ...
  const [editingAdminFor, setEditingAdminFor] = useState(null);
  ...
  return (
    <div style={{ padding: '20px', maxWidth: 1000, margin: '0 auto', width: '100%' }}>
      ...
      {/* Admins tab */}
      {tab === 'admins' && currentUser.role === 'Super Admin' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
            <button
              onClick={() => { setShowCreateAdmin(true); setAdminForm({ name: '', email: '', password: '' }); setAdminError(''); }}
              style={{ background: '#DC2626', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
            >
          ...
          {showCreateAdmin && (
            <div style={{ background: '#FEF2F2', borderRadius: 12, border: '1px solid #FECACA', padding: 20, marginBottom: 20 }}>
              <h4 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: '#991B1B' }}>New Admin Account</h4>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 16 }}>
                <div>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '9px 12px', border: '1.5px solid #CBD5E1', borderRadius: 7, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                  />
                </div>
                <div>
              ...
              {adminError && <p style={{ margin: '0 0 12px', padding: '9px 11px', background: '#FFF1F2', border: '1px solid #FECDD3', borderRadius: 7, color: '#B91C1C', fontSize: 12, fontWeight: 600 }}>{adminError}</p>}
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  style={{ padding: '9px 20px', background: savingAdmin ? '#FCA5A5' : '#DC2626', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 700, cursor: savingAdmin ? 'wait' : 'pointer' }}
                >
                  {savingAdmin ? 'Saving...' : 'Create Admin Account'}
                </button>
                <button
                  disabled={savingAdmin}
                  onClick={() => setShowCreateAdmin(false)}
                  style={{ padding: '9px 20px', background: '#fff', color: '#475569', border: '1px solid #E2E8F0', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: savingAdmin ? 'default' : 'pointer' }}
                >
                  Cancel
                </button>
              </div>
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8ECF0', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #F1F5F9', background: '#F8FAFC', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: '#0F172A' }}>{visibleAdmins.length} Admins</span>
            </div>
            {visibleAdmins.length === 0 && (
              <div style={{ padding: '40px', textAlign: 'center', color: '#94A3B8' }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>🛡️</div>
                <p style={{ fontWeight: 600 }}>{admins.length ? 'No matching admins.' : 'No admins yet. Click "Create Admin Account" above.'}</p>
              </div>
            )}
            {visibleAdmins.map(u => {
              const isEditingThis = editingAdminFor === u.id;
              return (
                <div key={u.id}>
                  <UserRow u={u} actions={<>
                    <button
                      onClick={() => {
                        setEditingAdminFor(isEditingThis ? null : u.id);
                        setEditAdminName(u.name || '');
                        setEditAdminPassword('');
                      }}
                      style={{ padding: '6px 12px', background: isEditingThis ? '#EFF6FF' : '#F1F5F9', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, color: isEditingThis ? '#0A6ED1' : '#475569', cursor: 'pointer' }}
                    >
                      Edit
                    </button>
                  ...
                  {isEditingThis && (
                    <div style={{ padding: '16px 20px 16px 72px', background: '#F8FAFC', borderBottom: '1px solid #F1F5F9' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 20, marginBottom: 16 }}>
                        ...
                        <div>
                          <label style={{ fontSize: 12, fontWeight: 700, color: '#475569', display: 'block', marginBottom: 6 }}>New Password (leave blank to keep current)</label>
                          <input
                            type="text"
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          onClick={async () => {
                            await updateEmployeeProfile(u.id, {
                              name: editAdminName,
                              ...(editAdminPassword ? { password: editAdminPassword } : {})
                            });
                            setEditingAdminFor(null);
                          }}
                          style={{ padding: '7px 16px', background: '#0A6ED1', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
                        >
                          Save Changes
                        </button>
                        <button
                          onClick={() => setEditingAdminFor(null)}
                          style={{ padding: '7px 16px', background: '#F1F5F9', color: '#475569', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                        >
                          Cancel
                        </button>
                      </div>
          ...
          <div style={{ flex: 1, minWidth: 0, width: '100%' }}>
            ...
            {currentUser?.id !== user.id && !currentUser?.isImpersonating && (
              <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
                <button onClick={handleProfileChat} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#0F172A', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: 8, fontSize: 14, fontWeight: 800, cursor: 'pointer' }}>
